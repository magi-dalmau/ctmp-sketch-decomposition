...
protected:
  ...
  Eigen::Affine3d target_object_pose_; //Object pose to be picked in world reference (it is not the pose to send to the IK solver for grasp it). It is used when checking the whole plan existance (no lazy valid action check) for setting a collision object.
